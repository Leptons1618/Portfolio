...
- **`image-upload.ts`** — `attachImageUpload(input, { dir, name })`, the upload-and-preview control every image path field on the admin mounts. The **second** caller of `commitFile` and the only one outside `content-store.ts`: it writes bytes to `public/`, which have no frontmatter, no schema and no slug, so routing them through the collection writer would buy nothing. It commits on pick rather than with the form — an orphaned image is harmless, a frontmatter path to a file that was never written fails `check-content.mjs`. It overrides the `value` setter on the input it is given, because `fillProject()`, the import prefill and every Revert assign `.value` directly and a property assignment fires no event.
