...
- **`npm run check:ai`** (`scripts/test-ai.mjs`) exercises the AI feature's security-relevant halves: that an API key cannot reach the admin listing (asserted against the **serialised** payload, because that is what leaves the Worker), that hidden projects and non-published posts never enter the assistant's corpus whatever the caller passes in, that the settings form cannot lift its own spending ceilings, and that the input caps hold. It deliberately does **not** assert that the scope prompt makes a model refuse an off-topic question — that is a property of a third party's weights, not of this code. It needs `scripts/ts-resolve.mjs`, a resolve hook that lets Node follow this repo's extensionless imports; that file explains why the alternatives are worse.
...
- **`ai_providers`** — one row per AI endpoint the site may call: a base URL, a model, an `active` flag and **the one API key this system stores**. Read only by `src/lib/ai.ts`; the admin listing gets a fingerprint and never the key — decision 22.
...
- **`ai-guard.ts`** — *what it may be asked.* The scope prompt, the input caps, and the rate limiter. Read its header before changing anything: it is explicit that the prompt is the **weakest** of the three defences and that the budget is the one that holds. Decision 23.
- **`assist-tasks.ts`** — the eight things the journal assistant can be asked to do, each with its own temperature, token ceiling and **allowlist of which editor fields are sent**. A closed table, not a forwarded prompt — decision 24.
