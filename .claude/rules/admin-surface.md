...
- **The parser's `!seenLabel` fallback is honoured only when the stream has finished.** `applyLive(..., final)` enforces that. Applied live it would write `TIT` into the body on the first delta and never correct it, because the fallback stops firing the moment `TITLE:` parses.
