...
**A token ceiling has to clear the thinking, and the model's own maximum is the default.** `max_tokens` bounds reasoning *plus* answer, so a number sized to the answer is a reasoning model that streams nothing at all — which is exactly what happened to `/write-whole-post` at 2,000. `ai_providers.max_output_tokens` holds what the model really accepts, filled in from the vendor's `/models` listing when a model is picked, and `effectiveMaxTokens()` **raises** every task ceiling to it and never lowers it. Nothing is billed for a ceiling that is not reached. Do not "tidy" that into a `Math.min`.
