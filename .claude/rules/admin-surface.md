...
**The Logs screen reads `/api/logs` and never renders a row server-side**, for the same reason the AI screen fetches its providers: a log line can quote a vendor's error, and since decision 64 the opening of what a model wrote. Rows are built with `createElement` and `textContent`, each with Copy and a two-click Delete; the foot pages both ways on keyset cursors (`before` / `after`), Export walks the current filter into a JSON download, and Clear is scoped to the filter (`DELETE /api/logs?source=&level=`, or `?id=` for one row). The table is written only by `record()` in `src/lib/log.ts`, and the rule in that file is the one to keep: **never a row per anonymous request** — every call site has authenticated the caller or spent a metered budget first, or a scanner could spend the free tier's D1 writes. The public assistant's per-run row is written from `agentStream`'s `onEnd`, which only runs after `charge()`. The error boundary's `reportFault()` is bounded to five a page for the same reason.
...
| --- | --- | --- | --- |
| `journal` | — | **save** only | `journal` status patch and row delete; the Daily journal tab patches the `journal-auto` row in `documents` |
...
| `logs` | — | **delete** only | rows of the `logs` table, over `/api/logs` — one, the filter's, or all; the error boundary files faults through it |
...
**Three journal screens, and the URL is the state.** `/admin/journal` is the manifest — every entry, search, filter, the status menu, delete — behind an *Entries* tab, with the daily journal's schedule behind a *Daily journal* tab beside it (`DailyJournalPanel.astro`, self-contained: its own `onAdminPage` root, Save button and provider read; the page only mounts it and tells it `dj-refresh` when its tab is shown). `/admin/journal/new` and `/admin/journal/[slug]` both render `src/components/JournalEditor.astro`; the only difference between them is the `slug` prop, `null` or the post's. That prop decides which path is written, whether the write is a create or a patch, and that **the filename does not follow the title** — Astro derives the slug from the filename, so renaming on a title edit would orphan a live URL. Do not reintroduce an `editing` variable: it was six pieces of UI kept in sync by hand, and decision 13 in `docs/DECISIONS.md` says why it is gone.
...
**The deep-dives line-up is the projects manifest itself, split in two by a rule.** `/admin/projects` renders one card per project in one of two regions: above the rule, the card leads the home page's Deep dives section; below it, it does not. Dragging a card across the line is the whole interaction — there is deliberately **no** deep-dive switch on the card, because which side it is on already says that, and a switch beside it would be the same fact written twice with two ways to disagree. The grip carries the keyboard equivalent: arrows reorder inside a region, Enter crosses the line. **The boundary is `.pm-band`, and it takes `--sep-ink`/`--rule-style`** — the theme's section-boundary pair, not `--color-divider`, which is the weight of the card edges either side of it. `SectionSep` is a *public page* mark and does not belong here; its hatch is invisible on the light themes anyway. Order inside the top region is the order the section renders in, and Save writes it as one document into the `projects-deep-dives` singleton, which `getDeepDiveProjects()` reads. It is the manifest rather than a tab beside it because the flag it must agree with is the visibility switch on that very card (decision 55). **A hidden project cannot lead** — `moveTo()` bounces one back down with the reason on its own status line, and retiring a card in the top region sends it down in the same gesture. **Automatic must move cards in both directions** — it saves the empty list, and the read side takes that to mean *the projects with a case study*, so a card left above the line because nothing moved it down would be a manifest disagreeing with the empty selection just saved. Rows were case studies until decision 53, and that is exactly what let a hidden project keep a card on the site.

**The card is `AdminProjectCard.astro`, and its styles live with it.** The manifest renders it from two regions, and a component root does not carry the caller's `data-astro-cid` — a `.pm-card` rule written in the page would silently match nothing.
...
**The journal editor's markdown preview is the post's renderer, and there is no second one.** `POST /api/preview` calls `renderBody()` — the same function, processor and `safeMarkdown` walk `POST /api/content` calls on the way into `body_html` — and the pane assigns the returned string to a `.prose` node, so the HTML *and* the styles are the page's. Do not reintroduce a markdown parser, a subset or a `.preview-md` type scale on this surface: the preview agreeing with the page is the whole feature (decision 67), and a client-side renderer is one that will disagree the first time the page's plugin list changes. The pane is debounced and run-guarded because it is a round trip per pause in typing; fenced code is lit by `code-fx.ts`, imported lazily by the same pane. The endpoint is behind `requireOwner()` — signed out, the pane says so rather than fetching a 401.
...
**The AI screen is two tabs — Providers and Public assistant — and the schedule is on the Journal screen.** It was a third tab here until decision 64; `DailyJournalPanel.astro` is that tab lifted whole. It writes the `journal-auto` singleton through the same one write endpoint; the *run record* (`journal-auto-run`) is deliberately **not** writable from the screen, because it is the endpoint's account of what it did and a form that could edit it could hand the job another day's attempts. Its number fields are server-rendered from `AUTO_DEFAULTS` — filling them needs a session, and a signed-out fork showed three empty boxes. "Run now" is the only caller that may `force` a run, and it does not spend one of the day's automatic attempts.
...
When a modal `<dialog>` is already open — the import form on `/admin/projects`, which is where "Draft with AI" raises the panel — `src/lib/modal.ts` steps that dialog down to modeless for as long as the panel is up, so the two coexist and both stay usable (decision 59). The page behind them is `inert` and veiled by a fixed scrim on `<body>` (`data-asx-freeze`, at 64, the colour of a modal's backdrop) — a veil, not the brightness filter it used to be, which read as a broken screen (decision 64). The stepped-down dialog and the toast host both move off the column a docked panel reserves.
...
**Nothing from the model is inserted as HTML.** The widget's markdown pass builds *elements* with `createElement` and fills them with `textContent`; links are turned into anchors **only when the href is a site-relative path**, because a model talked into emitting an absolute URL would otherwise get a clickable off-site link rendered in this site's chrome. Two `innerHTML` assignments exist across the authoring screens: the Mermaid preview in `assist-panel.ts`, whose comment says why, and the markdown preview pane in `JournalEditor.astro`, whose string is `renderBody()`'s output from `POST /api/preview` — raw HTML escaped and unsafe schemes unwrapped on the server, before the string exists. Never the model's own text passed through.
