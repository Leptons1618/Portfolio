...
| Import modal: search, four filters, three honest states | ✅ | `GET /user/installations/{id}/repositories` + the public listing. Each row carries what that listing already answered — language, stars, last push, licence, topics — because deciding between twenty repositories is what the dialog is for |
| Import → form prefilled from GitHub → one write | ✅ | Seven of nine fields come from the repo; `category` and `highlights` cannot, so **Draft with AI** in the form runs the same `/write-frontmatter` task the project page does, on the repository facts and its README. Creation only — editing is the detail page |
