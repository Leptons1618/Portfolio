...
| Output ceiling from the model's own listing | ✅ | `max_output_tokens`, filled in the moment a model is picked in the browser. It **raises** every task's ceiling and never lowers it, because `max_tokens` bounds thinking plus answer and a ceiling sized to the answer is a reasoning model that streams nothing — decision 37 |
| Reasoning effort, per provider and per run | ✅ | `reasoning_effort` where the vendor implements it. Absent by default, which is not the same as sending a vendor default |
