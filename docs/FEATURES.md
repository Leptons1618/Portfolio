...
| Tasks are slash commands | ✅ | `/write-whole-post`, `/draft-outline`, `/expand-selection`, `/tighten-prose`, `/write-summary`, `/revise-post`, `/suggest-titles`, `/suggest-tags`, `/draw-diagram`, `/describe-image`, `/rewrite-selection`, and `/write-frontmatter` + `/write-case-study` on the project screen. Written out per task rather than derived from the label, and *looked up* — a slash naming nothing is a message in the panel, never a prompt sent to a model |
...
| Only conversation carries the transcript | ✅ | The thirteen commands are a function of the draft; sending the history on each would be the whole conversation billed twelve times an afternoon. `assistPrompt()` trims it to twelve turns of four thousand characters and passes them as real `user`/`assistant` messages |
...
| Fourteen writing tasks | ✅ | Eleven in the journal — write the whole post, outline, expand selection, tighten selection, write summary, revise the post, suggest titles, suggest tags, draw diagram, describe image — and three on the project screen. `src/lib/assist-tasks.ts` — a **closed** table, each with its own temperature, token ceiling and allowlist of editor fields, because an authenticated endpoint forwarding arbitrary prompts is a general-purpose model on the owner's billing account (decision 24) |
