...
| An agent framework | ✂️ | Cut on purpose, and re-examined twice — when the assistant grew to a second surface, and when it became a conversation. A chat window is not an agent: the task table is still closed, a command is still looked up rather than interpreted, and nothing the model says can call anything. A closed task table, one `fetch`, one pure parser and the page's own form. Structured output is refused specifically: a JSON object is not readable until its last brace lands, and the whole point is that the form fills in while the response is still arriving. Decision 31 |
