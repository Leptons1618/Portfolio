...
**Status:** accepted — a third tab, the daily journal's schedule, was added by decision 52 and moved to the Journal screen by decision 64; the screen is two tabs again.
...
**Amended by decision 64.** The *look* of the freeze was a `brightness()` filter on those parts, and a page turned dark grey read as a broken screen. It is a fixed scrim on `<body>` now — the same veil a lone modal's `::backdrop` draws, at 64, under the stepped-down dialogs (65) — and the `inert` half is unchanged.


## 60. The answer round is not a lookup, long work is several requests, and a stream is not accepted before it says something

Decision 56 gave a run one deadline and folded an expired clock into the round limit: out of time, the model is told it has no more lookups and asked to answer from what it holds. The telling worked; the asking never happened. The disarmed round went out through `callChat()` carrying **the same expired deadline**, and `callChat()` — correctly, by its own rule — refused to start an attempt with no time left. So every run that ran out of time ended in `No time left in this request to try another`, *after* every lookup and every token of deliberation had been paid for. The same happened when the clock ran out while a lookup was running, which is the common case: a GitHub read is seconds, and slow free reasoning models spend ten or more per round before they ask for one. Reported as "all the tokens are spent before any output, mostly when it fetches details from the repo".

**The deadline decides when a lookup may begin; the answer gets its own.** `answerCall()` is the only way a disarmed round is sent, and it replaces the deadline with `now + ANSWER_GRACE_MS` (15s). That bounds the provider walk and the wait for headers, exactly as the run budget does, and like it never cuts a stream in flight. The run remains bounded: one lookup window, the round already in flight when it closed, and the grace. When the clock runs out *during* a lookup, the tools are withdrawn there too and the model is told in the last tool result — not in a separate `user` turn, which one or two strict routers still refuse after `tool` turns.

**Three smaller things made the window run out in the first place,** and they were fixed with it because each one spent the same budget:

- **The README was fetched twice.** The project screen sends it as context on every repository task, and the prompt told a model with `read_repo_docs` to "start here". It did — a round and eight thousand re-sent characters to read the document it had just been handed. `read_repo_docs` is now `skipWhenReadmeGiven`, withdrawn when the run carries the README, and the varying half of the prompt says the README is already there. A weak model obeys a tool list more reliably than an instruction, so both.
- **The repository readers fetched what the result cap then threw away.** Six documents at 3,000 characters each against an 8,000-character result cap meant the fourth to sixth were read, waited for, and cut. Now four at 1,800, read two at a time, and every GitHub read has an 8-second timeout. The tree listing went from 400 entries to 200 so it fits the same cap without losing its "N more not listed" line.
- **What a model said before a lookup went into the post.** "Let me read the README first." streamed as answer text, and the editors write answer text into fields. On a round that still has tools, the first `HOLD_CHARS` (240) are held: if the round ends in a lookup they are sent as thinking, otherwise they stream as the answer, a beat late.

**Long work is a sequence of requests, sequenced by the page.** A slow model doing four rounds of lookups and then 1,500 words in one request is racing a platform limit it cannot win — measured against OpenRouter's free pool, a 1,500-word request was still streaming at ninety seconds, while a ~350-word section finished in four to thirty-six. So the long case study is a plan (`casestudyplan`, the only step that looks anything up, and only in the repository) and then one request per section (`casestudysection`, given the plan's facts, the whole outline, its own heading and length, and the end of the section before it). The page runs them (`case-study-writer.ts`), retries a failed step once — twelve seconds later when the vendor said "rate-limited" — and keeps what finished when a run stops, so Resume carries on from the section that failed. Both steps are ordinary closed table entries: `step: true` keeps them off every menu and out of the request router, and nothing a caller sends can make a step anything but itself. This is decision 31's line held, not moved: the page decides the sequence, not a model.

**A 200 is not an answer, and a 403 is not always the key.** Tested live, the free pool's commonest failure is a stream that opens and carries only an `error` frame, and one model refuses with 403 "only available on agentic harnesses". The walk returned on the first and treated the second as a dead credential, so neither reached a fallback. `callChat()` now reads a streamed response to its first meaningful frame before accepting it — text, reasoning, a tool call or a finish reason, with a ten-second floor so a round begun near the deadline is not failed for the vendor's first-token latency — and replays the bytes it read, so readers downstream see the whole stream. A 403 ends a provider only when its body names the key, a credential or credits.
...
## 64. A run is logged, a text tool call is a lookup, and the schedule lives with the journal

**Status:** accepted

**Context.** One afternoon on OpenRouter's free pool with `nvidia/nemotron-3.5-lightning:free` as the only model on the row produced three failures and one blind spot. The daily journal refused two forced runs — "the model answered in no recognisable shape" and "the model spent this whole run thinking — about 12,182 tokens of deliberation and no answer" — and the log row for each carried `{day, attempt, caller, ms}` and nothing about what the model had returned. The writing assistant, on `/write-frontmatter`, ran two lookups, was told its third was out of time, and then wrote its next lookup **as text** — `<tool_call><function=read_repo_file>…</tool_call>` in the answer channel — which the panel showed as the answer with "the model answered without using the field format". Successful runs left no row at all, so there was nothing to compare a failure against. And the schedule that produced the two daily failures was configured on the AI screen, where nobody looking for a journal setting looked.

**Decision.** Four things, all in `src/lib/ai.ts` and the routes that call it.

**A tool call written as text is a lookup request, not an answer.** `textToolCalls()` reads the two shapes the open-weight models write — the tag-per-parameter form above and `<tool_call>{"name": …, "arguments": {…}}</tool_call>` — and returns calls only when at least one block parsed *and* the text outside the blocks is shorter than `HOLD_CHARS`; an answer that mentions a call in passing is an answer. Offered tools, the calls run exactly as `tool_calls` from the API would, with synthesised ids. With tools withdrawn, the model is told once more, in a `user` turn — there is no call id a `tool` turn could answer, and a `tool` message that follows no `tool_calls` is refused by the strict routers — and asked again on the answer round's own clock. Once per run (`nudged`); a second text call after the nudge ends the run with a sentence saying so. On the streamed path the opening of *every* round is now held while it is a prefix of `<tool_call>`, and a held block goes out as `thinking`, never as `delta`, so no editor field receives the markup. An ordinary answer still streams from its first character.

**A deliberation-only round is retried with reasoning off.** The one-time handover to the next model on the row (decision 60) does nothing on a row with one model, which is the common case on a free key. `retryFor()` is the two-rung ladder: the next model where there is one, otherwise the same model with `reasoning_effort: none`. `none` joins `EFFORT_LEVELS` — OpenRouter maps it to reasoning disabled on every model that allows it, Groq and the newer OpenAI models take it natively, and a model that cannot stop reasoning answers 400, which is an attempt failing like any other. It is offered in every effort picker as "Off". The daily job keeps `low` as its first try; the retry is a generation spent only when the failure has already happened.

**Every run leaves one log row.** `agentStream()` tallies the frames on their way out — answer and thinking characters, lookups started, the stop reason, an error, and token counts where a chunk carried `usage` (read where present, never asked for: `stream_options` is a body field a strict vendor 400s on) — and calls the route's `onEnd` from the generator's `finally`, so a client that hangs up still reports. `/api/ai/assist` writes the task, model, effort and the tally, and for a `document` task whether the answer parsed into fields at all, with its first six hundred characters when it did not — the line the panel shows as "not in the field format", kept where it can be read back. `/api/ai/chat` writes the model, timing and counts for every *charged* run and never the question or the answer: the provider is only called after `charge()`, so this is inside decision 62's rule and bounded by the day's budget. The daily job's refusals carry a `ComposeError` whose detail is the model, the stop reason, the counts, and the opening of what it wrote.

**The schedule is a tab on the Journal screen.** `DailyJournalPanel.astro` is the AI screen's third tab lifted whole — markup, script, its own Save button, its own read of the provider rows for the model picker — and `/admin/journal` renders it behind an *Entries / Daily journal* tablist. What it reads and writes did not move. `/admin/ai` is two tabs again, which is decision 44's original shape.

**Consequences.** `check:ai` pins the parser against both shapes and against prose, that the streamed loop nudges once and never streams `<tool_call` as a delta, that `agentComplete` nudges once and then throws, that a one-model row is retried at `none` and a row already at `none` is not, and that `onEnd` fires on the loop, on the shortcut path, and past a throw of its own. The freeze behind two dialogs (decision 59) is a fixed scrim on `<body>` at 64 rather than a brightness filter on the page — a dark grey page read as broken; a veil reads as "something is open over this" — and the toast host moves off the dock column with the stepped-down dialogs.

**Rejected.** Stripping the tool description from the system prompt once the tools are withdrawn — a rebuilt prompt per round, and the models that write calls as text have the syntax from their own template, not from the prompt. Sending `reasoning_effort: none` on the daily job's first try — a mandatory-reasoning model would then fail every day rather than the odd one. `ctx.waitUntil` for the log write — the same write, harder to test than a `finally`. Letting the browser post a run summary to `/api/logs` — the public widget cannot be a log writer. A `/admin/journal/daily` route — `[slug].astro` would lose any post slugged `daily` to it.


---

## 65. The import is a screen, and the case study is written after the project

**Status:** accepted

**Context.** Importing was two stacked modal `<dialog>`s on the projects manifest. The repository list closed itself to open the form, so there was no way back to the list, no URL for either step, and the browser's back button did not close anything. Escape on the form discarded thirteen prefilled fields with no warning. Create was disabled until the form differed from what the open had put there, which meant a fully prefilled import could not be created without editing something first. And `resolveCaseStudy()` ran **before** `createProject()`, so a failed create left a case-study row for a project that was never written — an orphan with no page and no card. The codebase already had the argument against all of this written down: identity was moved out of a modal to `/admin/settings` for exactly these reasons (decision 14).

**Decision.** Import lives at `/admin/projects/import`, a static route beside `[slug].astro`. The repository list and the form are two sections of one document rather than two dialogs: choosing a repository reveals the form below the list and puts the repository in the URL (`?repo=owner/name`), so a reload or a back press lands on the same form. The list's search and filter sync to `?q=`/`?show=` with `replaceState` — typing is not a navigation. After a good create the browser goes to `/admin/projects/<slug>?created=1`, and the project's own page shows the import note; there is no card on the import screen to update, which is what the old flow's "reload this screen" message was working around.

**Order is the fix for the orphan.** `createProject()` runs first. Only then is the case study written and `patchProject(slug, { caseStudySlug })` links it. A failure in the case study is reported as a partial success with a link to the project's page, not as a failed import — the project is the thing being created and the study is an attachment to it.

**The dirty guard is at the click, not at `before-preparation`.** Astro's router does not cancel on `preventDefault()` at `astro:before-preparation`; it falls back to a full page load of the destination, which discards the edits anyway. The router's own click handler bails when the event is already `defaultPrevented`, so a capture-phase listener asks first and hands the navigation to `navigate()` when the answer is yes. `beforeunload` covers the full-page case, and a `popstate` that closes the form pushes the entry back rather than asking a question whose "no" has nowhere to go. "Touched" is deliberately not the same question as "can this be created": the prefilled form *is* worth writing, so Create is live from the moment the form opens, and only an edit or a streamed draft marks the import as work that would be lost.

**Consequences.** `projects.astro` loses the two dialogs, the assistant mount and the whole import script — it is the manifest and the line-up again. `import.astro` owns the list, the form, the assistant and the create. `/admin/projects/import` wins over `[slug].astro`, so a project whose slug is literally `import` has no detail page; no imported title slugifies to that. Verified in a browser against a stubbed GitHub: rows in all three states, form restore from `?repo=`, filter sync, the dirty guard's accept/dismiss paths, and cancel.

**Rejected.** A route per repository (`/admin/projects/import/<owner>/<name>`) — the repository facts come from the browser's GitHub token, so a server-rendered form cannot be rebuilt from a URL without a second, tokenless read that answers less. Keeping the modals and driving them from the history API — the same code with more states to keep honest. Creating the case study first and deleting it on a failed create — a delete that can fail is a worse orphan-maker than the ordering that avoids it.

---

## 66. A run says what it cost, a lookup shows what it returned, and the panel keeps working during a run

**Status:** accepted

**Context.** The stream already carried `{usage:{prompt,completion}}` — decision 64 reads the vendor's counts where they appear — and every reader dropped it: `readStream()` had no `usage` branch, so the numbers existed only inside a log row's JSON. `readUsage()` read two fields of a block that can also say how much of the prompt came from the vendor's cache (`prompt_tokens_details.cached_tokens`, `cache_read_input_tokens`, `prompt_cache_hit_tokens`), and those were discarded. Prices were parsed for the model picker (`normaliseModels()` normalises to USD per million) and stored nowhere, so no screen could say what an answer cost. A tool row was one ellipsised line: the result the model was handed — the reason the answer says what it says — never reached the browser. The composer disabled its input for the whole run, so a follow-up prompted by a streaming answer had to wait or be lost. And a reopened conversation kept none of its per-message actions, because `restore()` rendered bodies only.

**Decision.** Five things, all additive to the frame protocol and the panel.

**Counts and cost ride on the usage frame.** `TokenUsage` gains `cached`, read from whichever of the three vendor spellings answers. The frame becomes `{usage:{prompt,completion,cached?,cost?}, model?}` — `model` a sibling, not a member, because `usage` is the vendor's block and the run tally stores it whole for the log. `cost` is computed per round from the prices on the row that answered and is **absent** when the row is unpriced: a free model and an unknown one must not read alike.

**Prices are columns on the provider row.** `migration/0013_ai_prices.sql` adds `price_prompt` and `price_completion` (REAL, nullable, USD per million), filled by the AI screen at the same moment the output ceiling is — one listing payload, one pick — and clamped on read by `clampPrice()` (negative refused: OpenRouter's `-1` means "depends which model this routes to"). The estimate charges cached tokens at the full input rate, because the discounted cache rate is a field `normaliseModels()` does not read; the panel prints it with a `≈` and the tooltip says it is an upper bound. The vendor's dashboard remains the authority.

**A lookup's result opening is a preview.** The outcome frame gains `preview`, capped at 600 characters server-side. The panel renders it behind a collapsed `<details>` on the row. It is the same published content the model was handed, so it cannot reach a reader that `curl` could not; the cap is because this is a trace, not a second copy of a `read_post`.

**The composer stays typeable during a run.** Enter queues instead of dropping: the line is held, the input clears for the next one, the hint says how many are waiting, and `running(false)` drains the queue in order. Closing the panel keeps the queue rather than discarding words the author typed.

**A reopened answer keeps the actions that still make sense.** `restore()` renders Copy on every assistant message, and Try again on plain conversation turns, re-sending the question above it. Page-owned actions (Insert, Undo, chips) are deliberately not restored — their targets are fields that may have moved on since, and an Insert into a stale form is worse than no Insert.
...
**Status:** accepted; Graphite and Herbarium reversed by decision 69. The Blueprint and Paper work here stands, and is extended by 69.

**Extends decision 7's mechanism to five themes — two of which decision 69 removed a week later. Nothing in that decision is reopened: a theme is still a token override file plus the marks tokens cannot express, and a page still never knows which one is on.**
...
**Consequences.** `THEMES` held five, the header toggle cycled them, and the `themeColor` pairs followed the new grounds. Three comments that said "the three themes" now say "the themes". The `--plate-*` tokens are set per theme, so the journal's photograph-less card is drawn in each theme's own stock — a consequence decision 69 undoes, since only the two removed themes set them. No new dependency, no new request, no markup change: the entire feature is three stylesheets and one array.
...
## 69. Two themes, drawn rather than rendered; the bar is pinned; a private repository says so

**Status:** accepted. Reverses part of decision 68 (Graphite and Herbarium are removed). Extends decision 7's mechanism and nothing else in it: a theme is still a token override file plus the marks tokens cannot express, and no page still knows which one is on.

**Context.** Three things happened at once.

1. **Five themes was two too many.** Graphite and Herbarium shipped three days before this and neither is on a page. Herbarium was the only serif here and the only theme with a drop cap, and the two between Geometry and Paper were the same two grotesque faces at different temperatures. What separated them from Paper and Blueprint was a palette, not an argument — and a palette is what a theme already has a token for. They are removed: `THEMES` goes back to three, the two stylesheets and Herbarium's reference go with them, and `--plate-*` falls back to the shared layer, which is what the other two themes already did.
2. **Paper and Blueprint were drawn, not rendered.** Both described a sheet somebody worked on by hand and then drew every line with a mathematical border. A dashed `border-top` is a mathematically dashed line; it is the one thing on either sheet that no hand drew. Neither had a filter, neither had a handwriting face, and Paper's ground was a beige rectangle with a uniform noise over it.
3. **The bar scrolled away, and a private repository read as an oversight.** The header was `position: relative` on a long page, so every section was far from the navigation and every route was one scroll away. And a project whose repository is private rendered no Repo button at all — correct as a link, and a visitor reads the missing button as a bug rather than as an answer.

**Decision.** Six things.

**The two hand-drawn themes draw.** A single `<svg>` in `BaseLayout` defines two `feTurbulence` + `feDisplacementMap` filters — `om-pencil` for a brush and `om-nib` for a pen — and the themes apply them to their rules, card outlines, tags and buttons. The `seed` is fixed in each filter, because a filter that re-rolled per paint makes the page shimmer under the reader and a hand-drawn page that never settles is not a page anyone can read. A filter displaces every pixel of the box it is on, so anything that carries text takes its outline from a filtered pseudo-element instead: a `filter` on a card would set the paragraph in the hand that was meant for its border. That is why `PencilDefs.astro` is a component and not a comment.

**The annotation tier is written, the document is not.** Paper's `§3` marks and `fig. 4 ·` captions are set in Caveat; Blueprint's `§1` marks, its `FIG. 01 ·` callouts and its `h1` are set in Architects Daughter, the reference's own lettering block. Both faces are `@import`ed *by the theme file* and apply to the pseudo-element only, so the request is tied to `data-theme` actually being set — a face declared in a layout loads on every theme, and one declared in a theme and used by a page never loads at all. The label beside a mark stays mono. A `§` in a hand beside a typed label is a drawing; a `§` in the same face as the label is a footnote.

**The stock is made of three layers, not one.** Paper adds a second, low-frequency turbulence (0.008) to the shared grain, so the mottling comes in patches the size of a thumb — which is what uneven pulp does and what one frequency cannot. Blueprint adds the same idea to the cyanotype at 0.006 over its ruled grid, because a chemical print's density varies across the sheet. Both data URIs take their colour out with `feColorMatrix saturate 0`: a data URI cannot carry a theme colour, and the ink has to come from the layer's opacity over whatever ground is beneath.

**The bar is pinned, and the ground runs under it.** `.site-header` is `position: sticky` at `z-index: 3` with `background: inherit`, so the body's own ground — the isometric mesh, the ruled sheet, the laid lines — is one sheet rather than a band that stops where the page does. The header's rail nodes are suppressed, because a node hanging off a pinned bar's bottom rule floats over whatever scrolled under it, and a pinned bar no longer meets `main`. `Header.astro` publishes `--header-h` from a `ResizeObserver` and `html` carries `scroll-padding-top` from it, so an anchor lands below the bar at any width without a hard-coded number — the bar wraps below 960px and a late webfont moves it. The case study's reading progress moves into the header through a named slot: it was `position: fixed` from inside `<main>`, which is a `z-index: 1` stacking context, so a pinned bar covered it. Sticky is screen-only; a bar repeated at the top of every printed page is not a bar.

**Below 640px the links scroll sideways.** A pinned three-row bar measured 177px on a 390px screen — a fifth of it, held back on every scroll. A hamburger was rejected before the bar was pinned, and a drawer would hide the navigation these themes' whole argument is about, so the links stay visible and the row scrolls, with a mask so the cut edge reads as "there is more" rather than as a clipped word.

**A private repository is stated.** A project with no `repoUrl` renders a dashed, muted `Private repo` mark in the Repo button's place — the same `.btn` box, so an actions row keeps its rhythm from card to card, and plainly not a control: no fill, no hover, no pointer. The detail page and the case study say the same thing. The import screen stops writing the URL at all: `fieldsFromRepo` drops `repoUrl` for a private repository, because a repository that is private now and public in six months should get its link from the owner typing one in, not from a stale row. Migration 0011's content-not-toggle rule holds — an absent field is still the whole of the state.

**Consequences.** `THEMES` holds three and the toggle cycles Geometry → Blueprint → Paper. A visitor whose stored `om-theme` is `graphite` or `herbarium` falls back to Geometry through the pre-paint script, which has always validated a stored id against the list; the stale string sits in storage until they pick a theme. Two new font dependencies, both `@fontsource`, both imported by one theme file each. No page changed. The colophon's "3 themes × light/dark" and "three themes, one component layer" were already true and are true again.

**Rejected.** A torn-paper deckle on the body — a 2px dashed rule at the viewport edge is a dashed rule, not a torn edge, and a page taller than any sheet has no edge to tear. Displacing text with the filter, for the reason above. A Scroll-Driven `animation-timeline` to fade the bar in on scroll: it is a progress bar, and the site's own reading-progress bar is one of the marks this site draws better than a gradient. A `is-stuck` class for the bar's shadow — a scroll listener and a class to maintain for a shadow, when the bar's own top rule is already on the viewport edge.

**Revisit if** the theme list grows past three again. The mechanism does not care, but the reason to delete Graphite and Herbarium was that they were palettes rather than arguments, and that question is worth asking of the next one.

---

## 70. The editor's verbs are pure functions, and the preview shows the post

**Status:** accepted. Extends decision 67 (the preview is the post's renderer) and decision 13 (Write and Preview are tabs). Both stand; this is what they left out.

**Context.** Two gaps, and the first one made the second invisible.

The editor's toolbar had five buttons and they only ever *wrapped*. Pressing Bold on already-bold text produced `****word****`, an empty selection left the caret after both markers, and there was no heading, list, quote, fence or table at all — so every one of those was typed by hand. There were no keyboard bindings except the assistant's own. `setRangeText` kept the undo stack, but the AI panel's `/rewrite-selection` had no case in `prepare()` and fell through to the default, so a task whose own description said "It replaces the selection" appended the rewrite to the end of the post. A hero upload set `input.value` programmatically, which fires nothing: `trackDirty` never heard about it, Save stayed disabled, and the image was silently not on the row.

The preview was the post's *body* and not the post. Decision 67 made the body the real renderer, and the head was left as a lone `h3` at 25px — not the page's title, not its scale, no hero, no tags, no meta line, no summary. So the one screen an author checks before saving was showing them a document with none of the things a reader sees first.

**Decision.** Four things.

**Every transform is a pure function.** `src/lib/md-edit.ts` exports `toggleInline`, `toggleHeading`, `toggleList`, `toggleLink`, `insertFence`, `insertTable`, `insertImage`, `insertRule`, `continueList`, `indentLines` and `stats`, each taking `(text, start, end)` and returning an edit. No DOM, no imports that touch `import.meta.env`, so `npm run check:editor` exercises all of it under `node:assert` like the rest of this repository's tests. The DOM layer in `JournalEditor.astro` is reduced to "apply the edit and move the caret", which is the only part that could be wrong in a way a string comparison catches.

**Every verb toggles.** The toolbar's old contract was *wrap*, and a wrapping button has no off state: the second press makes the post worse and the author fixes it by hand. Unwrapping is tried from outside the selection first and from inside second, because a selection gets its markers either way. An empty selection produces a marker pair with the caret *between* them, so the next keystroke lands inside the bold. Block inserts are padded to a paragraph boundary — `text\n## Heading` is a lazy continuation in CommonMark, not a heading, and an author who cannot see their own heading working blames the editor.

**The shortcuts are the ones already taught.** Ctrl+B/I/E/K and Ctrl+Shift+X, Tab/Shift+Tab, Enter to continue a list and to leave an empty one. Each button and each shortcut calls the same `runVerb`, so they cannot drift, and each button's `title` carries its binding. Escape releases the Tab trap: a textarea that eats Tab forever fails WCAG 2.1.2, and Escape is the only key that gives the trap up without taking a modifier. Ctrl+Shift+P is deliberately *not* the preview toggle — Firefox takes it on Windows, and Ctrl+Shift+V is paste.

**The preview shows the head too, and shares the page's CSS for it.** `.post-head` and its five children moved out of `src/pages/journal/[slug].astro`'s scoped block into `global.css`, and the preview renders the same markup with the same classes. A *copy* of a post head is a preview that drifts from the page it previews, which is the exact failure decision 67 removed from the body; one stylesheet, two surfaces. The head is painted from the form fields on every render and never touches the server, so it is right on the first paint, on every keystroke, and while signed out — the three cases where a round trip cannot help. The body still goes to `renderBody()`, because that is the only thing that can be right about markdown.

**Consequences.** `npm run check:editor` is chained into `npm run check`. A hero upload, a library pick and a Clear all mark the form dirty, because `image-upload.ts`'s value setter now dispatches an `input` event — one line in the one place every image field on the admin goes through. `/rewrite-selection` replaces, and refuses to replace a range whose text has moved underneath it: the panel is non-modal, so the author can type during a run, and an offset-based replace would have destroyed whatever moved. A save clears the autosave timer as well as the key, because a save completing inside the 700ms debounce left a timer that re-wrote the snapshot of a post already in the database. 500 words reads as "500 words · 2,500 chars · 2 min" under the toolbar; pressing it fills the Read time field.

**Rejected.** A WYSIWYG surface — a rich text editor is a second representation of the document, and this one has a server-side renderer as its single source of truth. A `contenteditable` body with a markdown serialiser: the same problem, plus the caret and undo behaviour to rebuild. A find-and-replace panel: real, and not in this pass — the shortcut layer is the thing a textarea genuinely lacks. Split view: decision 13 rejected it for width, and a 1440px admin gives the two columns about 400px each, which is the same answer. Body image paste and drop: wanted, and it needs `uploadImage()` pulled out of `image-upload.ts`'s closure first, so it is a separate change rather than a half-wired one here.

**Revisit if** the editor ever needs to write two files at once — decision 13's own trigger, which would put the state back in a variable.

---

## 71. What the hand-drawn themes actually cost, and the one number that is worth acting on

**Status:** accepted. Extends decision 69; nothing in it is reopened. This is the record of a measurement, including the part of the result that did not explain itself.

**Context.** Two of the three themes scroll noticeably worse than Geometry, Blueprint worst, and it got worse when 69 landed — which is when the SVG filters, the paper fibre and the cyanotype mottle arrived. The obvious suspects were those. They are not the whole answer, and saying so is most of this decision.

**Method.** A deterministic scroll, because the obvious one lies. `rAF` deltas are vsync-locked and read 16.7ms for a page that is visibly stuttering, because the compositor can drop work and still present a frame. Wheel momentum also differs per theme, so the three themes did not receive the same scroll. What was used instead: exactly 60 steps of `scrollTo` with two `rAF`s between each, identical for every theme, traced with `devtools.timeline` and read for `RunTask` on `CompositorGpuThread`. Main-thread cost was measured too, with `Performance.getMetrics` deltas, and came out flat across all three themes (TaskDuration 0.41s / 0.48s / 0.49s for the same work) — which is the first real finding, because it says the problem is not JavaScript.

**Result, reproducible over repeated rounds.** GPU time for the 60-step scroll of the 21-card grid: **Geometry 71ms, Blueprint 120ms, Paper 145–176ms.** The gap is real and it is raster.

**What it was not.** Toggling each candidate at runtime moved nothing outside noise: the SVG filters (115 → 114), `box-shadow` (→ 111), the hand-lettered fonts (→ 112), the grain pseudo-element (→ 111), the inner sheet frame (→ 115), the 12px grid reduced to 48px (→ 120), the turbulence tile removed (→ 124). Removing the body background made it *worse*, 116 → 181, and the reason is in the next section.

**The one thing worth fixing.** `filter` gives every element it is on its own render surface. Measured on `/projects`: Geometry had **0**; Paper had **227**; Blueprint had **186** — of which 145 in both were `.tag`, and in Paper another 41 were `.btn::before`. A displacement filter on a 10.5px two-character label, in a 70×28px button, was paying for something no reader could see. Those are gone. Paper and Blueprint are both at **41** passes — 21 rules and 20 card outlines — an 82% and 78% cut, with the tag now carrying a dashed border and the button a dashed one, which is what a hand draws around a label on a marked-up sheet anyway. The filter stays where the displacement is actually visible at the size it is drawn: card outlines, rules, the kraft separator.

**This did not move the number on this machine, and that is the honest part.** The tags were 145 render surfaces, and removing them changed GPU time by about 1ms. It is still the right change and it is still the one that matters most, because the cost is per-paint and this machine is fast: at 145 filter passes a modern GPU absorbs them, and a phone or a low-power laptop does not. The thing a developer notices as "geometry is smooth" is very likely the 49ms of GPU work that geometry does not have, and the *worst* of it is the part that is worst per unit of hardware rather than the part that is worst in absolute milliseconds here.

**What is still unexplained.** The residual 49ms is not attributable to any single property. Hiding the page header and filter row takes Blueprint 120 → 57ms and Geometry 72 → 18ms, so the gap is real and lives in that region — but within it, the `PathLine` costs ~38ms and the header illustration ~29ms *in every theme*, and `content-visibility`, the filters, the fonts, the grid and the frame all fail to account for the difference. The remaining suspects are the interaction of the shared chrome with each theme's token values, which would need a bisection of `blueprint.css` itself rather than of the rendered page. Not done. Recorded rather than guessed at.

**`background-attachment: fixed` stays.** It looks like the obvious suspect and it is the opposite: switching it to `scroll` costs **+77ms** (116 → 193). The fixed attachment is what lets the compositor cache the ground as a layer and translate it. It is also what makes the pinned header's `background: inherit` show one continuous sheet instead of a band that stops where the page does, so it is load-bearing for 69 as well as cheap.

**Rejected.** `content-visibility: auto` on the cards — the textbook answer for a long grid, and it worked: Paper 176 → 144ms. But `contain-intrinsic-size` has to guess a card's height until that card is measured, and the guess was ~50px off, so the document shrank from 2833px to 2782px *as you scrolled through it*. A slow scroll traded for a scrollbar that moves is not a performance fix. The rejected rule is left as a comment in `src/pages/projects.astro` so the next person does not run the experiment again. Also rejected: trimming the 12px grid to a coarser step (no measurable effect — the cost is not in the ground), and dropping the hand-lettered fonts (none). Nothing was tuned on a number that did not move.

**Consequences.** Paper and Blueprint draw 41 render surfaces per paint instead of 227 and 186. No visual change a reader would name: a tag and a button lost a sub-pixel wobble they never had at their size. Geometry is untouched. No measurement threshold is asserted in CI — the numbers above come from one machine, and a gate built on them would be a gate on that machine.

**Revisit if** the residual is worth chasing on hardware that actually feels it. The bisection this did not do is: disable `blueprint.css` wholesale and re-measure, then re-enable its rules a block at a time. That is the next experiment, and it needs a machine where the difference is visible without a trace.

---

## 72. The ground is an element, not a background attachment

**Context.** "Blueprint's background moves while scrolling." Not a judgement about taste — a measurement, and the first one in this sequence that came out reproducible on the first try. Geometry was measured in the same pass and was least affected (0.24 against Blueprint's 1.27), which is exactly the trap: a theme that is merely *acceptable* on a fast machine is the same mechanism waiting for a slower one, and the fix is only cheap while it is still one line of CSS. Geometry is included here rather than left behind.


**How it was measured, and what that cost.** Two attempts were wrong before this one was right, and both are worth recording because the failure mode is a harness that reports success.

`rAF` deltas are vsync-locked and read 16.7ms for a page that visibly stutters. Puppeteer's `screenshot({ clip })` takes its rectangle in *document* coordinates, so a clip at `y: 250` samples the same pixels at scrollY 1000 and at 1500 — the first harness reported a background as "byte-identical across scroll" when it had never scrolled. `html { scroll-behavior: smooth }` makes `scrollTo` animate, so two calls 1px apart landed on the same intermediate frame. And comparing raw PNG *bytes* reported "99.3% of bytes differ per 1px", which is a statement about zlib, not about the page.

What worked: `scroll-behavior: auto` in the test, `waitForFunction` on `scrollY`, a full-viewport capture, and — the part that matters — decoding the PNG with `createImageBitmap` into an `OffscreenCanvas` and diffing **pixels**. The metric is the mean absolute per-channel difference between the viewport at scrollY and at scrollY+1, with content hidden so the sample is pure ground.

**The result.** Geometry 0.24. Paper 0.42. **Blueprint 1.27.** Delete Blueprint's grid and it is 0.002. Halve the line frequency and it halves. Coarsen the grid to the 60px major lines alone and it is 0.47.

**What that means.** `background-attachment: fixed` does not paint a small cached layer — it tells the browser to repaint a *body-sized* background anchored to the viewport, and on a document 2800px tall that is re-rasterised against every scroll offset. The visible cost is proportional to how many lines the ground has, which is why this read as the jittery theme and Geometry did not: a 56px mesh has a fifth of Blueprint's line count, and 1.27/4.7 ≈ 0.27 ≈ Geometry's 0.24 exactly. Nothing was broken. A fine grid on a screen is just a thing that moves when it is redrawn.

**Decision.** All three grounds move off the body and onto a `position: fixed` pseudo-element at `z-index: -1`, with the body's background cleared. A fixed *element* is rasterised once and only ever composited after that. Same test, same 12px rule, same 60px module: **Blueprint goes from 1.27 to 0.000**, Paper to 0.02–0.04, and **Geometry from 0.24 to 0.009.** Every grid keeps its scale; the sheets stop sliding.

**The bar had to be given a ground of its own.** This is the part the screenshots caught and the numbers did not. With the ground behind all content, `.site-header`'s `background: inherit` resolved to nothing and the pinned bar became transparent — the cards slid *through* the navigation. Each theme now paints its own gradients on an opaque `background-color` in the header, which lines up because a stuck header's background box and the fixed layer are both anchored to the viewport's top-left. The sheet runs unbroken through the bar and the bar is opaque to what scrolls under it. The shared layer's `background: inherit` is now dead for all three themes and is kept only as the default.

**The raster tiles moved too, and this one is a correction.** 71 moved the fibre and the mottle from a `background` tile onto the fixed grain overlay, on the theory that a non-integer tile was seaming. That theory was wrong: a test that reinjected the old 420px tile and compared pixels found it was already stable, and the seam was not there. The move still stands, on the better reason that a raster tile is the one background layer that genuinely cannot be re-evaluated at paint time, but it is not what fixed anything here.

**Consequences.** Ground drift, mean per-channel per pixel of scroll, repeated: **Geometry 0.009, Blueprint 0.000, Paper bimodal between 0.00 and 0.63.** Paper's is the one loose end — it is intermittent rather than steady, it appears identically with the fibre layer removed, and each of its three ground layers alone measures 0.000, so it is not any one of them. It is recorded rather than claimed as fixed. GPU time over a 60-step programmatic scroll: Geometry 64ms to 71ms, Blueprint 99ms to 183ms, Paper to 161ms. That is the cost of rasterising a viewport-sized layer once rather than a document-sized one sixty times, and spread over 60 frames it is roughly 3ms each against a 16.7ms budget. The per-frame budget, not the aggregate, is what a reader feels.
